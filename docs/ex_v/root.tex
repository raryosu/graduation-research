...
クラスタリングを行う手法である．
k-meansの逐次繰り返しと，BIC\cite{schwarz1978estimating}(Bayesian Information Criterion; ベイズ情報量規準)による
...
$d$次元のデータ${\bm D}=({\bm x_0}, {\bm x_1}, \cdots, {\bm x_i})$を
$K$個のクラスタに分割することを想定すると，
$d$変量等方Gauss分布\eqref{eq:gaussian-distribution}を定義できる．
\begin{align}
...
なお，分散$\sigma^2$は\eqref{eq:variance}により表される．
\begin{align}
  \label{eq:variance}
  \hat{\sigma}^2 = \frac{1}{R-K}\sum_i\left(x_i - \mu_{(i)}\right)^2
\end{align}

この時，事後確率$Pr[M_j \mid {\bm D}]$\footnote{[私的メモ]${\bm D}$を仮定した時$M_j$の事後確率}を
モデルの評価に用いたい．
そこで，事後確率を近似し正規化したBICを用いる．
\begin{align}
  \label{eq:bic}
