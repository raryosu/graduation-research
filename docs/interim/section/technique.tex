...
偶然を伴う現象は，ある確率分布に従う確率変数の実現地であると考えることができる．
...
モデルに関する真の分布のK-L情報量は\eqref{eq:k-l-div}で表される．
\begin{align}
  \label{eq:k-l-div}
  I(g \mid f) &= E_X \log \left\{\frac{g(X)}{f(X)}\right\}\\\nonumber
              &= \int^{\infty}_{-\infty}\log\left\{\frac{g(x)}{f(x)}\right\}
\end{align}
ただし，$\log$は自然対数で，注記がない限り一貫してこの意味で用いる．

このように，真の分布がわかっている場合にはK-L情報量によってモデルの良し悪しを比較できた．
しかし，通常は真の分布は未知で，真の分布から得られたデータだけが与えられていることが多い．
...
\begin{align*}
  I(g \mid f) &= \int^{\infty}_{-\infty}\left\{\log\frac{g(x)}{f(x)}\right\}g(x)dx\\\nonumber
              &= \int^{\infty}_{-\infty}\{\log g(x)\}g(x)dx - 
                 \int^{\infty}_{-\infty}\{\log f(x)\}g(x)dx
\end{align*}
となるが，右辺の第一項は定数であり，したがって右辺第二項が大きいほどK-L情報量$I(g \mid f)$は
小さくなることがわかる．右辺第二項の$\int^{\infty}_{-\infty}\{\log f(x)\}g(x)dx$は，
...
与えられているものとする．
$\theta$は密度関数を規定するパラメータである．この時，観測値$(x_1, x_2, \cdots, x_n)$は
...
\subsection{情報量規準}
情報量規準とは，最尤推定によって当てはめられたモデルが複数個あるときに，
その中の一つを選択する規準である．
